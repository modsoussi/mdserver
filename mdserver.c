 ...
 *
 *      !!! On fork():
 *          -  The child process has its own copy of the parent's descriptors.(V IMPORTANT) These descriptors reference the same 
 *             underlying objects, so that, for instance, file pointers in file objects are shared between the child and the parent,
 *             so that an lseek(2) on a descriptor in the child process can affect a subsequent read or write by the parent.
 *             This descriptor copying is also used by the shell to establish standard input and output for newly created
...
void gen_response_header(char *resp, req_hdrs_t *req_hdrs){
    ...
    if(!(strstr(req_hdrs->method, "GET") || strstr(req_hdrs->method, "HEAD"))){
		strcpy(req_hdrs->path, "./notimplemented.html");
    ...
    if((fd = open(req_hdrs->path, O_RDONLY, 0)) < 0) {
		...
		memset(req_hdrs->path, 0 , sizeof(req_hdrs->path));
		if(errno == EACCES){
			strcpy(req_hdrs->path, "./forbidden.html");
			sprintf(resp, "HTTP/1.1 403 Forbidden\r\nServer: %s\r\nContent-Type: text/html\r\nConnection: close\r\n", SERVER);
      } else {
...
void req_handle(int sd, req_hdrs_t *req_hdrs){
  ...
  if(strcmp(req_hdrs->method, "HEAD") != 0){ /* only send file if GET */
